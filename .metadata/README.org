...
Commands would be like a few bytes, sent to one IP: one byte for command, one byte for prism id, two for arguments.
Commands would be e.g.
Beep (no arg, no arg),
Calibrate to bottom position (prism, no arg),
set target (prism, position),
start move (prism, no arg),
stop move (prism, no arg),
report position (prism, no arg),
set parameters (param id, param value)
...
| write-target-position      | '<BBBH'        |             0x01 |           0x0B | prism-address position-mm |       0x0B | '<B'            |
| write-all-target-positions | '<BBHHHHHHH'   |             0x01 |           0x0C | position-mm * 7           |       0x0C | '<B'            |
| pause                      | '<BBB'         |             0x01 |           0x0D | prism-address             |       0x0D | '<B'            |
| pause-all                  | '<BB'          |             0x01 |           0x0E |                           |       0x0E | '<B'            |
| resume                     | '<BBB'         |             0x01 |           0x0F | prism-address             |       0x0F | '<B'            |
